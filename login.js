...
class NeumorphismLoginForm {
    ...
    prefillForm() {
        const localAuth = localStorage.getItem('sgu_auth');
        const sessionAuth = sessionStorage.getItem('sgu_auth');
        
        // Already logged in? Redirect to dashboard
        if (localAuth || sessionAuth) {
            try {
                const auth = JSON.parse(localAuth || sessionAuth);
                if (auth.email) {
        ...
        // Pre-fill email and Remember me if localStorage auth exists
        if (localAuth) {
            try {
                const auth = JSON.parse(localAuth);
                if (auth.email) {
                    this.emailInput.value = auth.email;
    ...
    getFirstName(email) {
        if (email === 'monzertarigabdalla@gmail.com') return 'Monzer';
    ...
    async handleSubmit(e) {
        ...
        if (password !== 'monzer') {
            this.showError('password', 'Invalid password.');
            this.animateSoftPress(this.submitButton);
            return;
        }
        ...
        try {
            ...
            const firstName = this.getFirstName(email);
            storage.setItem('sgu_auth', JSON.stringify({
                email: email,
                name: firstName,
                firstName: firstName,
                loggedInAt: Date.now()
    ...
    showNeumorphicSuccess() {
        ...
        // Redirect handled in handleSubmit
        setTimeout(() => {
            window.location.href = 'index.html';
        }, 2500);
    }
